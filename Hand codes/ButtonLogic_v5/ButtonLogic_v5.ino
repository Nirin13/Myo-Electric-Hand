...
int final_pos = 180; // servo maximum degree to pull a string

int lastsensorState = HIGH;   // the previous reading from the input pin
unsigned long lastDebounceTime = 0;  // the last time the output pin was toggled
unsigned long debounceDelay = 30;    // the debounce time; increase if the output flickers
bool sensorstate = HIGH; //saving state of the switch
...
void setup() {
  ...
  threshold = sens_min + ((sens_max - sens_min) * 0.4);
...
void loop() {
  ...
  if ((millis() - presstime) > 500 && sensorstate == 1) //wait for some time and if sw is in release position
  {
  ...
  {
    pinky.write(final_pos);
    ring.write(final_pos);
    middle.write(final_pos);
    index.write(final_pos);
    thumb.write(final_pos);
  }
